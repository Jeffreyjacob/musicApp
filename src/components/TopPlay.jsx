...
import { useGeTopArtistQuery, useGetTopChartsQuery } from '../redux/services/lastFm';


const TopChartCard = ({song,i,isPlaying,activeSong,
  handlePauseClick,handlePlayCheck})=>(
  <div className="w-full flex flex-row items-center
   hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
   <h3 className="font-bold text-base text-white
   mt-3">{i + 1}</h3>
   <div className="flex-1 flex flex-row justify-between items-center">
     <img className="w-20 h-20 rounded-lg mx-3"
      src={song?.album?.cover_medium} alt="song?.title"/>
...
const TopPlay = () => {
  ...
  const {data:topArtist} = useGeTopArtistQuery();
  ...
  const TopArtist = topArtist?.data?.slice(0,5)
        ...
        <div className="mt-4 flex-col gap-1">
          {
            TopPlays?.map((song,i)=>(
              ...
              handlePlayCheck={()=>handlePlayCheck({song,i})}
              />
            ))
           }
        </div>
          ...
          TopArtist?.map((song,i)=>(
            <SwiperSlide key={i}
             ...
             <Link to={`/artists/${song?.id}`}>
              <img src={song?.picture_big} alt={song?.name}
